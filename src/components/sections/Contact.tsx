...
function ContactLinkCard({
  ...
  const { icon: Icon, label, value, href, color, glow } = link;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      cardRef.current.style.transform = `perspective(600px) rotateX(${y}deg) rotateY(${x}deg) translateZ(10px) scale(1.02)`;
      cardRef.current.style.boxShadow = `0 20px 50px rgba(0,0,0,0.3), 0 0 25px ${glow}`;
      cardRef.current.style.borderColor = `${color}35`;
      cardRef.current.style.background = `${color}08`;
    },
    [color, glow]
  );
  ...
  return (
    <a
      ...
      style={{
        ...
        transition: "opacity 0.5s ease, transform 0.4s cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease, background 0.3s ease",
    ...
    >
      ...
      <ExternalLink
        ...
        style={{ color, transform: "translateZ(12px)" }}
      />
    </a>
  );
}
...
export default function Contact() {
  ...
  return (
    <section id="contact" ref={sectionRef} className="relative py-32 overflow-hidden">
      ...
      {/* 3D Mandala watermark */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-4 preserve-3d">
        <MandalaRingIcon size={600} className="text-violet animate-mandala-3d" />
      </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6">
        ...
        >
          {/* Glass Card with Rotating Trishul Background */}
          <div className="glass p-8 md:p-12 rounded-3xl relative overflow-hidden" style={{ border: "1px solid rgba(255,107,26,0.15)", boxShadow: "0 0 40px rgba(255,107,26,0.05)" }}>
            
            {/* Large faint rotating trishul in background */}
            <div className="absolute top-2/3 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-5 flex items-center justify-center">
              <div 
                className="preserve-3d"
                style={{ color: "#ff6b1a", animation: "mandala-spin-3d 20s linear infinite" }}
              >
                <BigTrishulBg size={500} />
              </div>
            </div>
            ...
            <div className="relative z-10">
              ...
              <div className="text-center mb-10">
                <div
                  className="flex justify-center mb-4 preserve-3d"
                  style={{
                    color: "#ff6b1a",
                    animation: "glow-pulse 3s ease-in-out infinite, mandala-spin-3d 10s linear infinite",
                  }}
                >
                  <TrishulIcon size={40} />
                </div>
